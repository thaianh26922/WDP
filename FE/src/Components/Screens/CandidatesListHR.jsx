...
function CandidatesListHR(props) {
    const [candidate, setCandidate] = useState([]);
    const [status, setStatus] = useState('');
    useEffect(() => {
        async function getAllAppliedJobs() {
            try {
                const res = await axios.get('http://localhost:9999/api/apply-job/get-all-applied-jobs').catch(err => console.error(err));
                if (res) {
...
export default CandidatesListHR;